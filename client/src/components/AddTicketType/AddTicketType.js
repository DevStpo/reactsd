...
class AddTicketType extends Component {
  ...
  handleChange = e => {
    if(["name", "fieldType"].includes(e.target.name)) {
      let id = e.target.id.split("-")[1], fields = [...this.state.fields];
  ...
  render() {
    ...
    const options = workflows.map(workflow=>({value: workflow._id, label: workflow.name}));
    ...
    return (
      <div>
        ...
        {fields
          ...
          .map((field, idx) => {
            let fieldId = `field-${idx}`, nameId = `n-${idx}`, typeId = `t-${idx}`;
            return(
              <div key={fieldId}>
                <TextField
                  label="Field Name"
                ...
                />
                  ...
                  <IconButton aria-label="Delete" className={classes.margin} onClick={this.onDelete.bind(this, idx)}>
                    <DeleteIcon fontSize="small" />
                  </IconButton>
              </div>
            );
        })}
        <DataSelect options={options} label="Workflow" handleChange={this.setWorkflow} />
        <Button variant="contained" color="primary" className={classes.button} onClick={this.onAddField}>
          Add a Field
        </Button>
...
export default connect(mapStateToProps, { addTicketType, getWorkflows })(withStyles(styles)(AddTicketType));
