...
import { getTickets, getTicket, deleteTicket } from '../../actions/ticketActions';
...
class TicketsList extends Component {
  ...
  handleSelectAllClick = e => {
    if (e.target.checked) {
      this.setState(state => ({ selected: this.props.ticket.tickets.map(ticket => ticket._id) }));
      return;
    }
  ...
  render() {
    ...
    const { views, currentView = {} } = this.props.view;
                        ...
                        {fields.map(field =>
                          (field.apiName === 'title')
                            ? <TableCell key={field._id}>
                                <Link to={`/ticket/${ticket._id}`} onClick={this.onTicketClick.bind(this, ticket._id)}>
                                  {ticket.fields[field.apiName]}
                                </Link>
                              </TableCell>
                            : <TableCell key={field._id}>{ticket.fields[field.apiName]}</TableCell>
                        )}

                        <TableCell>
                          <Tooltip title="Delete">
                            <IconButton aria-label="Delete">
                              <DeleteIcon onClick={this.onDeleteClick.bind(this, ticket._id)} />
                            </IconButton>
                          </Tooltip>
                        </TableCell>
                      </TableRow>
...
export default connect(mapStateToProps, { getTickets, getTicket, deleteTicket, getViews, setCurrentView })(withStyles(styles)(TicketsList));
